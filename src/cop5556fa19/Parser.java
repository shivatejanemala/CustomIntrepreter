...
private Exp andExp() throws Exception{
	...
	private Block block(Kind... end) throws Exception {
		...
		Stat e1  = null;
		List<Stat> statementlist = new ArrayList<Stat>();
		while(!isKind(end) && !isKind(KW_return)) {
			 e0 = statBlock();
				if(e0!=null) {
					statementlist.add(e0); 
				}	
	...
	private Stat statBlock() {
		...
		try {
			switch(t.kind) {
				...
				case NAME : case LPAREN:{
					...
					if(isKind(ASSIGN)) {
						...
						e0 = new StatAssign(first, nameList, null);
					}
				break;
				}
				case KW_local:{
			...
			System.out.println("Error caught while creating AST for given block of statements-"+e);
		}
		return e0;
	}
	...
	private Stat labelBlock() throws Exception {
		...
		return new StatLabel(first, new Name(first,first.text),null,0);
	}
		...
		switch (t.kind) {
			case LPAREN:{
				...
				if(!isKind(RPAREN)) {
					...
					/*while(isKind(RPAREN)) {
						List<Exp> ee = new ArrayList<>();
						ee.add(e2);
						e2 = new ExpFunctionCall(first, e1, ee);
					}*/
					if(e2!=null) {
						args.add(e2);
					}
				...
				/*else {
					Exp e2= new ExpFunctionCall(first, e1, new ArrayList<>());
					args.add(e2);
				}*/
				match(RPAREN);
			}break;
